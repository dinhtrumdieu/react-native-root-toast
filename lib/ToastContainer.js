...
class ToastContainer extends Component {
    ...
    _renderSuccessView = () => {
        ...
        {backgroundColor: 'rgba(45,172,145, 0.8)'}
    ]}>
        {this.props.icon ? this.props.icon : <SvgXml xml={SuccessIcon} />}
        <Text
    ...
    _renderFailedView = () => {
        return (
            <View style={[styles.container, {backgroundColor: 'rgb(247,73,64)'}]}>
        {this.props.icon ? this.props.icon  : <SvgXml xml={ErrorIcon}/>}
        <Text
            allowFontScaling={false}
            numberOfLines={4}
            style={styles.message}>{this.props.children}
                </Text>
                </View>
        )
    }
